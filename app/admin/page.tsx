import { getAllOeuvres, getOeuvresAVendre } from "@/lib/oeuvres";
import Link from "next/link";

export default function AdminDashboard() {
  const all = getAllOeuvres();
  const vente = getOeuvresAVendre();
    ...
    <div>
      ...
      <div className="mb-10">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            href="/admin/oeuvres"
            className="border border-[#3D3A36] text-[#D4C9B6] text-sm tracking-widest uppercase px-6 py-3 hover:border-[#C8A96E] hover:text-[#C8A96E] transition-colors"
          >
            Gérer les œuvres
          </Link>
          <Link
            ...
            <tbody>
              {all.slice(0, 8).map((o) => (
                <tr key={o.slug} className="border-b border-[#3D3A36]/50 hover:bg-[#3D3A36]/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[#C8A96E]">
                    {o.prix ? `${o.prix} €` : "—"}
                  </td>
